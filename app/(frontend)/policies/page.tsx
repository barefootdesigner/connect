  ...
  return (
    <div className="px-6 sm:px-12 lg:px-24 py-8 sm:py-12 lg:py-20">
      ...
      <div className="mb-12 sm:mb-16 lg:mb-24 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-gray-900 pr-16 sm:pr-0">Policies & Procedures</h1>
        <SearchBar />
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-3xl shadow-[8px_8px_24px_0px_rgba(0,0,0,0.12)]">
          <p className="text-slate-600">No policies available yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10">
          {policies.map((policy) => (
            <FileCard
              key={policy.id}
              fileName={cleanFileName(policy.filename)}
              fileType={getFileType(policy.file_type)}
              fileSize={formatFileSize(policy.file_size)}
